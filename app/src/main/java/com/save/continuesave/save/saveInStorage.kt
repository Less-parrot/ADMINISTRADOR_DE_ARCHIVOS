...
fun saveInStorage(folder: List<String>) {
    ...
    val folders = remember { mutableStateListOf<String>() } // Lista mutable para mantener las carpetas
    ...
    Box(Modifier.fillMaxSize()) {
        Box(Modifier.fillMaxHeight()) {
            OutlinedTextField(
                ...
                onValueChange = { userCode ->
                    texto = userCode //hacemos que el texto sea lo que se escriba en el campo de texto
                },
                ...
                keyboardActions = KeyboardActions {
                    ...
                    currentFocus?.let {
                        inputMethodManager.hideSoftInputFromWindow(it.windowToken, 0)//cerramos teclado
        ...
        if (state) {
            ...
            state = false //hacemos que state vuelva a ser false, para no entrar en un bucle infinito
        }
...
fun listFoldersInDocuments() {
    ...
    val folders: MutableList<String> = mutableListOf() //lista que almacena el nombre de todas las carpetas
    ...
    if (documentsDirectory.isDirectory) {
        ...
        for (directory in listFiles) { //ahora listamos archivos que estén en las carpetas
            if (directory.isDirectory) {
                val folderName = directory.name //lista de nombres de los archivos que estén en las carpetas
                val filesInDirectory = directory.listFiles()

                for (file in filesInDirectory) {
                    if (file.isFile) {
                        val diccionario = "$folderName: ${file.name}"//nombre de carpeta : archivo
                        ...
                        val nameFile = map[nameFolder]//usamos el nombre seleccionado de las carpetas para hacer el filtro
                        if (nameFile != null) {
                            Log.d("nombreArchivos", nameFile.toString()) //imprimimos por LogCat el nombre de el archivo
                        }
...
fun createFolderInDocuments(nameFolder: String) {
    ...
    if (!folder.exists()) {//verificamos que no exista la carpeta [nameFolder] en la carpeta DOCUMENTS
        if (folder.mkdirs()) {
            ...
            Toast.makeText(contexto, "Carpeta $folderName fue creada con exito", Toast.LENGTH_SHORT).show()
            val nameFile = "archivo.py"
            val file = File(folder, nameFile)  //¡FALTA: poner que el usuario elija el nombre de el arhcivo a crear!
            if (!file.exists()) {
                try {
                    if (file.createNewFile()) {
                        // El archivo se creó con éxito.
                        Toast.makeText(contexto, "Archivo '$nameFile' fue creado con exito", Toast.LENGTH_SHORT).show()
                    } else {
                        // Ocurrió un error al crear el archivo.
                        Toast.makeText(contexto, "Ocurrió un error al crear el archivo '$nameFile'", Toast.LENGTH_SHORT).show()
                    }
                }
                catch (e: IOException) {
                    // ocurrio un error
                ...
                Toast.makeText(contexto, "El archivo '$nameFile' ya existe en la carpeta", Toast.LENGTH_SHORT).show()
            }
        } else {
            // Ocurrió un error al crear la carpeta.
            Toast.makeText(contexto, "Ocurrió un error al crear la Carpeta $folderName", Toast.LENGTH_SHORT).show()
        }
    } else {
